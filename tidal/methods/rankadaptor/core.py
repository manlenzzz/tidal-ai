...
class RankObservation:
    ...
    def prediction_error(self) -> float | None:
        if self.predicted_score is None:
            return None
        return abs(float(self.predicted_score) - float(self.score))
...
def online_incremental_rank_search(
    ...
    *,
    ...
    if convergence_tolerance is not None and convergence_tolerance < 0:
    ...
    def observe(config: RankConfig, source: str, predicted: float | None = None) -> RankObservation:
        item = RankObservation(dict(config), float(evaluate_config(dict(config))), source, predicted)
    ...
    for step in range(iterations):
        ...
        observation = observe(predicted_config, "online", float(predicted_score))
        convergence_error = observation.prediction_error
        if (
            convergence_tolerance is not None
            and step + 1 >= min_iterations
            and convergence_error is not None
    ...
    return OnlineRankSearchResult(
        dict(best.config),
        float(best.score),
        final_observations,
        dict(predicted_config),
